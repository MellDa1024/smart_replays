...
#Unused - This method returns the formatted name of the executable file, but it seems too dangerous... (I’m not familiar with windll operations)
#Code generated by ChatGPT (GPT-4o)
def get_file_version_info(file_path):
    try:
        info_size = ctypes.windll.version.GetFileVersionInfoSizeW(file_path, None)
        if not info_size:
            return None

        buffer = ctypes.create_string_buffer(info_size)
        ctypes.windll.version.GetFileVersionInfoW(file_path, None, info_size, buffer)

        def get_value(subblock):
            r = ctypes.c_void_p()
            l = ctypes.c_uint()
            res = ctypes.windll.version.VerQueryValueW(buffer, subblock, ctypes.byref(r), ctypes.byref(l))
            if res and r.value:
                return ctypes.wstring_at(r.value)
            return "Unknown"

        r = ctypes.c_void_p()
        l = ctypes.c_uint()
        if ctypes.windll.version.VerQueryValueW(buffer, r'\\VarFileInfo\\Translation', ctypes.byref(r), ctypes.byref(l)):
            lang_code = f"{ctypes.c_ushort.from_address(r.value).value:04x}{ctypes.c_ushort.from_address(r.value + 2).value:04x}"
        else:
            lang_code = "040904b0" #English

        product_name = get_value(fr'\\StringFileInfo\\{lang_code}\\ProductName')
        file_description = get_value(fr'\\StringFileInfo\\{lang_code}\\FileDescription')

        return product_name or file_description
